...
//        //создание потоков
       ...
       // После завершения потоков — вывод информации о студентах
System.out.println("\n" + "=".repeat(70));
System.out.println("ИНФОРМАЦИЯ О ВЫПОЛНЕНИИ ЛАБОРАТОРНОЙ РАБОТЫ");
System.out.println("=".repeat(70) + "\n");

try {
    Thread.sleep(500); // небольшая пауза перед выводом
} catch (InterruptedException e) {
    e.printStackTrace();
}

// Используем метод printWithDelay() для красивого вывода
printWithDelay("Лабораторная работа №1: Создание потоков", 100);
printWithDelay("Выполнил: Буймистр Денис", 100);
printWithDelay("Группа: CR-233", 100);
printWithDelay("Работа выполнена успешно!", 100);
 }
